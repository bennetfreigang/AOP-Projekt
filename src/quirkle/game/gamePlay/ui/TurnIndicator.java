 ...
 * is waiting for.
 *
 * @note Deliberately does not repeat the scores. Those are on the rack for the active player and
 *       on the cards for everyone else; the banner answers only "whose move is it, and now what".
 * @see TileRack for the active player's scores
 */
public class TurnIndicator extends PanelEntity {
    ...
    public TurnIndicator(Game game, int centerX) {
        this.game = game;
    ...
    public void onRender(Graphics2D g) {
        ...
        drawText(currentPlayer.getName().toUpperCase(), UiTheme.FONT_SIZE_TURN_NAME, UiTheme.GOLD,
    ...
    private String buildStatusLine() {
        ...
        return UiTheme.text("turn") + " " + game.getTurnNumber() + SEPARATOR + prompt;
    }
}
